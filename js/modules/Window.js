var Window = function(o) {
	...
	var reset = (param('reset') || Config.reset);
	...
	var getpos = function(o) {
		...
		if (reset || !save || !user.id || !user.pref || !user.pref.win)
			return 1;
	...
	var tab = function(t) {
		...
		j(o.id + ' .tab-content').append('<div id="tab-'+i+'" class="'+( t['class'] || t.name || '')+' tab-pane'+(i==0?' active':'')+'">'+(t.html||'')+'</div>');
