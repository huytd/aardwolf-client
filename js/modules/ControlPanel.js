var ControlPanel = function () {
	...
	var loadProfiles = function() {
		...
		j(id + ' .gamelist').append('<div class="profiles"><a class="folder" data="profile-link"><i class="icon-folder-open-alt"></i> My Profiles</a><br></div>');
		...
		for (var p in pref.profiles) {
			...
			j(id + ' .gamelist .profiles').append('<a class="profile-link" host="'+P.host+'" port="'+P.port+'"\
					thumb="' + ( d[i] ? d[i].img : '' ) + '" profile="'+p+'" name="'+P.name+'"><i class="icon-star-empty"></i> '+p+'<br></a>');
		}
	}
	
	var loadList = function(at, closed) {
		...
		for (var g = 0; g < d.length; g++) {
			...
			try {
				...
				var img = el.match(/"(images\\\/.+?\.(png|gif|jpg))"/i);
				
				d[g].img = '/' + (img ? img[1].replace(/\\\//g, '/') : '' );
				
				j(id + ' ' + at).append('<a class="game-link" ' + (closed ? 'style="display: none" ' : "") + 'data="'+g+'" host="'+d[g].host+'" port="'+d[g].port+'" thumb="'+d[g].img+'" name="'+d[g].name+'"><i class="icon-sun"></i> '+d[g].name+'<br></a>');
			} 
			catch(ex) { 
				console.log(ex, d[g]) 
			}
	...
	var loadAlphabeticalList = function() {
		
		if (Config.solo || Config.clean || touch)
			return;

		j(id + ' .gamelist .profiles').after('<div class="azlist"><a class="az-list folder"><i class="icon-folder-close-alt"></i> A-Z List<br></a></div>');
		...
		loadList('.azlist', true);
	}
	
	var loadSiteList = function() {

		console.log('loadSiteList');
		...
		var m = new Modal({
			...
			buttons: [{
				...
				click: function() {
					...
					var v = j('.modal .modal-body input').val();
	...
	var chatUpdate = function(i) {
		...
		var bit_1 = '<span style="opacity: 0.6">';
		var bit_2 = '</span> <span style="color: #01c8d4;opacity:0.7">';
		...
		if (!i) {
			...
			chatlog.map(function(i) {
				...
				if (mychannel == i[1].channel) {
					...
					var line = bit_1 + i[0].substr(11,5) + bit_2 + i[1].name+': </span>'+linkify(i[1].msg)+'<br>';
					
					if (i[1].msg.start('/me ')) {
						
						var msg = i[1].msg.split('/me ');
						msg.shift();
						msg = msg.join('/me ');
						
						line = '<span style="opacity: 0.6">'+i[0].substr(11,5) + bit_2 +i[1].name+' </span>'+linkify(msg) + '<br>';
					}
		...
		if (mychannel == i[1].channel) {
			...
			if (i[1].msg.start('/me ')) {
				...
				line = bit_1 + i[0].substr(11,5) + bit_2 + i[1].name+' </span>' + linkify(msg) + '<br>';
			}
	...
	j(id).on('keydown', '.chat-send', function(e) {
		...
		if (e.which == 13) { /* enter */
			...
			if (v.start('/name ')) {
				...
				user.username = v.split('/name ')[1];
				
				!Config.Socket || Config.Socket.write(stringify({
					name: user.username
				}));
				
				return chat.send(stringify({
					chat: 1,
					name: user.username,
					channel: 'op'
				}));
