/*
 * Socket from www.mudportal.com, slighty adopted to Freshman's MXP module
 * Version 2.0a    10/3/2020
 *
 * 2.0  - version compatible to MXP module
 * 2.0a - reset timeout raised from 1s to 2s; auto html link insertion avoid links already handled by MXP
 *        do not trim spaces when separator set to '' for file editing in the mud.
 */

var Socket = function(o) {

        var self = this, ws = {}, out = o.out || Config.ScrollView, connected = 0;
        var proxy = o.proxy || Config.proxy;
        ...
        var touthand;
        var no_mxp_timeout = true;

        if (proxy.has('play.aardwolf.com'))
            delete o.proxy;
        ...
        var onClose = function(evt) {
                ...
                if (out)
                        out.add('<br><span style="color: green;">The server has closed the connection. Refresh the page to reconnect.<br></span>');
        ...
        var onMessage = function (e) {
                ...
                log('Now processing, buff is now', buff);
                process();
        };

        var     onError = function(evt) {
                out.add('<span style="font-size: 13px; color: red;">Error: Telnet proxy may be offline.<br></span>');
        };
        ...
        var send = function(msg) {
                ...
                */
                ...
                if (ws.send && connected) {
                        if (out && !msg.has('\033[1z'))
                                out.echo(msg);
                        ws.send(msg + '\r\n');
                }
                else
                        if (out)
                                out.add('<span style="font-size: 13px; color: green;">WARNING: Connect first - refresh the page or click the two arrows in a circle icon.<br></span>');
        ...
        var sendMXP = function(msg) {

                if (!connected)
                        return console.log('attempt to sendMXP before socket connect');

                log('Socket.sendMXP: ' + stringify(msg));
                ws.send(stringify({ mxp: msg }));
        ...
        var process = function(force) {
                ...
                if (force) {
                        if(touthand) {
                                clearTimeout(touthand);
                                touthand = undefined;
                        }
                }
                if (!buff.length && (!Config.mxp || Config.mxp.idle())) {
                        no_mxp_timeout = true;
                        return;
                }
        ...
        var prepare = function(t, force) {
                ...
                if (t.match(/\xff\xfa[^\xff\xf0\x01]+$/) && !force) {
                        ...
                        buff = t;
                        //log(buff);
                        if(touthand) clearTimeout(touthand);
                        touthand = setTimeout(process, 1000, 1);
                        return;
                }

                // this MXP close detection by TAG ESC codes is too limited
                if (Config.mxp && Config.mxp.enabled()) {
                        // if there is anything looking like an open tag in..
                        // Not necessary to wait for an escape sequence to close..
                        // If incomplete, it will self 'resync' on next input.

                        // is there is any non escaped < or unclosed &entity in:
                        if (t.match(/^<|[^\033]<|&[a-z0-9_]*$/i) && !force) {
                                // we trigger a timeout.. however the mxp module handles partial input.
                                //console.log('mxp split protection triggered');
                                if(touthand) clearTimeout(touthand);
                                touthand = setTimeout(process, 2000, 1);
                                no_mxp_timeout = false;
                                // do not: return;
                        } else if (force && !Config.mxp.idle()) {
                                log('mxp split protection fired: reset mxp module');
                                t += Config.mxp.print2forcereset();
                                no_mxp_timeout = true;
                        }
                }
                ...
                /*
                if (t.match(/\x1b\[[1-7]z<dest/i) && !force) {
                        if (!t.match(/<\/dest/i) || (t.match(/<dest/gi).length != t.match(/<\/dest/gi).length)) {
                                buff = t;
                                return setTimeout(process, 2000, 1);
                        }
                } */
                ...
                // if we came here (no return)
                // there is a timeout running, but it was no MXP auto close protection
                // we can savely clear the timer.
                if(touthand && no_mxp_timeout) {
                        clearTimeout(touthand);
                        touthand = undefined;
                }
                ...
                // This is difficult as it easily interacts and screws mxp html links
                // a proper code would need to keep state to know if we are in a <A> already
                t = t.replace(/([^"'>\w])(https?:\/\/[^\s\x1b"'<]+)/g,'$1<a href="$2" target="_blank">$2</a>');

                t = t.replace(/\xff\xfa../g,''); // \xff\xfa is a 4 byte sequence
                t = t.replace(/\xff\xf0/g,'');
                t = t.replace(/\xff../g,'');
                t = t.replace(/\xff./g,'');
